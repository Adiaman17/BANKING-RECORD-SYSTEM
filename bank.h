...
class bank
{
    public:
      ...
      void newaccount(int f) // TO CREATE A NEW ACCOUNT
        {
            cin.get();
...
int check(int no) // TO CHECK WHETHER THE THE GIVEN ACCOUNT NUMBER EXISTS
{
    bank b;
...
void disp1(bank & b) // TO DIPLAY THE ACCOUNT DETAILS OF THE CUSTOMER
{
...
void passen(bank & b,char pa[],int ac,double with,double dep,double tot) // TO ADD DETAILS TO THE PASSBOOK
{
    int &x=b.top;
...
void create(bank & b) // TO CREATE AN ACCOUNT
{
...
void del(bank & b) // TO DELETE AN ACCOUNT
{
        bank ba;
        ifstream fin("bankmanage.dat",ios::binary);
        ofstream fout("temp.dat",ios::binary);
...
void withdraw(bank &b) // TO WITHDRAW MONEY FROM AN ACCOUNT
{
    double amm;
        if(b.type==3)
            {
...
void deposit(bank & b)  // TO DEPOSIT MONEY INTO AN ACCOUNT
{
    double amm;
    if(b.type!=4)
    {
...
void accset(bank & b) // TO MODIFY THE DETAILS OF AN EXISTING CUSTOMER
{
    int z;
    disp1(b);
    cout<<endl<<endl;
...
void interest(bank & b) // TO CALCULATE THE INTEREST AMOUNT
{
    if(b.type!=3)
        {
            double &d=b.days;
...
void dispass(bank & b) // TO VIEW THE PASSBOOK ENTRIES
{
    interest(b);
    cout<<"*******************************************************************************"<<endl;
...
void trans(bank & b) // TO TRANSFER MONEY
{
    int am,c,amt;
